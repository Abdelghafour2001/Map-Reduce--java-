    ...
    implements Tool
{
    ...
    public int run(String[] args) throws Exception
    {
        ...
        if (args.length != 2) {
            System.err.println("Usage: MaxHauteurDriver <inputpath> <outputpath>");
            System.exit(-1);
        }

        // créer le job map-reduce
        Configuration conf = this.getConf();
        Job job = Job.getInstance(conf, "MaxHauteur Job");
        job.setJarByClass(MaxHauteurDriver.class);

        // définir la classe Mapper et la classe Reducer
        job.setMapperClass(MaxHauteurMapper.class);
        /*job.setCombinerClass(MaxHauteurCombiner.class);*/
        job.setReducerClass(MaxHauteurReducer.class);
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        MaxHauteurDriver driver = new MaxHauteurDriver();
